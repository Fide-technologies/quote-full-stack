...
import { ImageIcon, CheckCircleIcon } from '@shopify/polaris-icons';
...
export const QuoteForm: React.FC<QuoteFormProps> = ({ 
    ...
    const currentStep = formState.steps[currentStepIndex];
    if (!currentStep && !isSubmitted) return null;

    if (isSubmitted) {
        return (
            <div className="animate-fadeIn text-center py-10 px-4">
                ...
                <button
                    ...
                    className="w-full bg-[#1a1c1d] text-white border-none py-4 px-6 rounded-lg text-sm font-bold cursor-pointer transition-all duration-200 hover:bg-[#2b2d2f] active:scale-95 shadow-md flex items-center justify-center uppercase tracking-wider"
                >
                    Continue Shopping
                </button>
            </div>
        ...
        <div className="animate-fadeIn">
            ...
            <div className="mb-8">
                <div className="flex justify-between items-center mb-3">
                    <Text variant="bodySm" as="span" tone="subdued" fontWeight="medium">
                        Step {currentStepIndex + 1} of {formState.steps.length}
                    </Text>
                    <Badge tone="info" progress="partiallyComplete">
                        {`${Math.round(((currentStepIndex + 1) / formState.steps.length) * 100)}% Complete`}
                    </Badge>
                </div>
                <div className="h-2 bg-[#f1f2f4] rounded-full overflow-hidden border border-[#f0f0f0]">
                    <div
                        ...
                        style={{ width: `${((currentStepIndex + 1) / formState.steps.length) * 100}%` }}
                    />
                </div>
            </div>
            ...
            <div className="mb-7 text-center">
                <h2 className="text-2xl font-bold text-[#1a1c1d] mb-2 tracking-tight">
                    {currentStepIndex === 0 ? 'Submit Your Quote Request' : currentStep.title}
                </h2>
                <p className="text-[#6d7175] text-sm leading-relaxed">
                    {currentStepIndex === 0 ? 'Fill in your details and we\'ll get back to you shortly' : (currentStep.description || 'Review and complete the following details.')}
                </p>
            </div>

            {currentStep.id === 'step-review' ? (
                <div className="mb-8">
                ...
                <div className="flex flex-wrap gap-y-5 justify-between">
                    {currentStep.fields.map((field) => (
                        <div key={field.id} className={field.layoutWidth === 'half' ? 'w-full md:w-[calc(50%-10px)]' : 'w-full'}>
                            <div className="mb-1">
                                ...
                                {field.type === 'textarea' ? (
                                    ...
                                    <div className="w-full">
                                        <div className="py-6 px-4 border-2 border-dashed border-[#d2d5d8] rounded-lg bg-[#f9fafb] text-center cursor-not-allowed transition-all duration-200">
                                             ...
                                             <div className="text-[#0066FF] font-semibold text-sm mb-1">{field.allowMultiple ? 'Click to upload multiple images' : 'Click to upload'}</div>
                                             <div className="text-xs text-[#6d7175]">
                                                 Supported: {[...(field.allowedImageFormats || []), field.allowedFileTypes].filter(Boolean).join(', ') || 'Images'} (Max {field.allowMultiple ? '3 images' : '1 image'}, {field.maxFileSizeMB || 5}MB)
                                             </div>
                                         </div>
                                    </div>
                                ) : (
            ...
            <div className="flex gap-4 mt-10">
                ...
                <button
                    type="button"
                    onClick={() => {
                        if (currentStepIndex < formState.steps.length - 1) {
                ...
                >
                    {currentStepIndex < formState.steps.length - 1 ? 'Continue to Next Step' : 'Submit Request'}
                </button>
            </div>
        </div>
    );
